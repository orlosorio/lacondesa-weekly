...
export default function NuevaAperturaPage() {
  ...
  const handlePhotoChange = (e: React.ChangeEvent<HTMLInputElement>) => {
    ...
    const valid = files.filter((f) => {
      if (!["image/jpeg", "image/png", "image/jpg"].includes(f.type)) return false;
  ...
  const handleSubmit = async () => {
    ...
    const res = await fetch("/api/submit-opening", { method: "POST", body: formData });
      ...
      <main className="pt-20 lg:pt-24 min-h-screen bg-background">
        <div className="max-w-xl mx-auto px-4 sm:px-6 lg:px-8 py-16 lg:py-24 text-center">
          ...
          <p className="text-muted-foreground leading-relaxed">
            Got it. If your place is a fit for the neighborhood, we&apos;ll be in touch soon.
          </p>
        </div>
      </main>
    );
    ...
    <main className="pt-20 lg:pt-24 min-h-screen bg-background">
      <div className="max-w-xl mx-auto px-4 sm:px-6 lg:px-8 py-12 lg:py-16">
        <header className="mb-12">
          ...
          <p className="text-muted-foreground leading-relaxed">
            We review every submission personally. If it&apos;s a fit for La Condesa, we&apos;ll reach out.
          </p>
        </header>
            ...
            <div className="space-y-4">
              ...
              <div>
                <label className={labelClass}>
                  Describe in your own words what makes your place special * (max {MAX_DESC})
                </label>
                <textarea
